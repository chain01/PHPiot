 ...
 * WebSocket protocol.
 */
class Websocket implements \PHPiot\Protocols\ProtocolInterface
{
    /**
     * Websocket blob type.
     *
        ...
        if (empty($connection->websocketHandshake)) {
            ...
            if (strlen($connection->tmpWebsocketData) > $connection->maxSendBufferSize) {
                if ($connection->onError) {
                    try {
                        call_user_func($connection->onError, $connection, PHPIOT_SEND_FAIL, 'send buffer full and drop package');
                    } catch (\Exception $e) {
                        Worker::log($e);
                        exit(250);
                    } catch (\Error $e) {
     ...
     *
     * @param string                              $buffer
     * @param \PHPiot\Connection\TcpConnection $connection
     * @return int
     */
    protected static function dealHandshake($buffer, $connection)
    {
        ...
        if (0 === strpos($buffer, 'GET')) {
            ...
            if (preg_match("/Sec-WebSocket-Key: *(.*?)\r\n/i", $buffer, $match)) {
                ...
                $connection->send("HTTP/1.1 400 Bad Request\r\n\r\n<b>400 Bad Request</b> for detail.",
                    true);
                $connection->close();
                return 0;
            }
            ...
            $handshake_message .= "Server: phpiot/".Worker::VERSION."\r\n";
