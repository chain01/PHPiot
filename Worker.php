...
use \PHPiot\Events\Libevent;
use \PHPiot\Events\Event;
use \PHPiot\Events\React;
use \PHPiot\Events\Select;
use \PHPiot\Events\EventInterface;
use \PHPiot\Connection\ConnectionInterface;
use \PHPiot\Connection\TcpConnection;
use \PHPiot\Connection\UdpConnection;
use \PHPiot\Lib\Timer;
use \PHPiot\Autoloader;
use \Exception;
...
class Worker
{
    ...
    /**
     * pid文件的路径及名称
     * 例如 Worker::$pidFile = '/tmp/PHPiot.pid';
     ...
     * 可以手动设置
     * 例如 Worker::$logFile = '/tmp/PHPiot.log';
     * @var unknown_type
     */
    ...
    protected static $_availableEventLoops = array(
        'libevent' => '\PHPiot\Events\Libevent',
        'event'    => '\PHPiot\Events\Event'
    );
    ...
    public static function init()
    {
        ...
        {
            static::$logFile = __DIR__ . '/../PHPiot.log';
        }
        // 标记状态为启动中
    ...
    public static function runAllWorkers()
    {
        ...
        if(count(static::$_startFiles) === 1)
        {
            ...
            if(count(static::$_workers) > 1)
            {
                ...
                echo "@@@ Please visit http://wiki.PHPiot.net/Multi_woker_for_win @@@\r\n";
            }
    ...
    {
        // 定时检查子进程是否退出了
        Timer::add(0.5, "\\PHPiot\\Worker::checkWorkerStatus");
    ...
    protected static function displayUI()
    {
        ...
        echo "----------------------- PHPiot -----------------------------\n";
        echo 'PHPiot version:' . Worker::VERSION . "          PHP version:".PHP_VERSION."\n";
        echo "------------------------ WORKERS -------------------------------\n";
    ...
    protected static function getEventLoopName()
    {
        ...
        if ($loop_name) {
            if (interface_exists('\React\EventLoop\LoopInterface')) {
                switch ($loop_name) {
                    case 'libevent':
                        static::$eventLoopClass = '\PHPiot\Events\React\LibEventLoop';
                        break;
                    case 'event':
                        static::$eventLoopClass = '\PHPiot\Events\React\ExtEventLoop';
                        break;
                    default :
                        static::$eventLoopClass = '\PHPiot\Events\React\StreamSelectLoop';
                        break;
                }
            } else {
            ...
            static::$eventLoopClass = interface_exists('\React\EventLoop\LoopInterface')? '\PHPiot\Events\React\StreamSelectLoop':'\PHPiot\Events\Select';
        }
        return static::$eventLoopClass;
    }
